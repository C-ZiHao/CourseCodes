...
int main(void)
{
	...
	while (1)
	{
		...
		for (num = 0; num < 8; num++)
		{
			result = I2C0_WriteByte(TCA6424_I2CADDR, TCA6424_OUTPUT_PORT1, seg7[num + 1]); //write port 1
			result = I2C0_WriteByte(TCA6424_I2CADDR, TCA6424_OUTPUT_PORT2, led[num]);	   //write port 2

			result = I2C0_WriteByte(PCA9557_I2CADDR, PCA9557_OUTPUT, ~led[num]);
