...
//@Data 2020.6.6  
//讨论6，时钟实现
//功能：利用八个数码管显示小时-分钟-秒，例09:18:59,在题目要求之外添加小时
//核心思想:常规状态，利用systick实现每秒的计时，并动态扫描显示时间
//         按下sw1，利用systick每200ms增加1s
//         按下sw2，利用systick每200ms增加1min
//         按下sw1与sw2，利用systick每200ms同时增加1s与1min
...
void Display(int count, int num);    //演示
...
int main(void)
{
	...
	while (1)
	{
		...
		case 2: number = 10;break; //number=10时，显示"-"
		case 3: number = minute / 10;break; //分钟
		case 4: number = minute % 10;break;
		case 5: number = 10;break; //number=10，显示"-"
...
void SysTick_Handler(void)
{
	...
	if (GPIOPinRead(GPIO_PORTJ_BASE, GPIO_PIN_0) == 0 && GPIOPinRead(GPIO_PORTJ_BASE, GPIO_PIN_1) == 0) 
	{
		...
		if (systick_200ms_status) //每200ms
		{
