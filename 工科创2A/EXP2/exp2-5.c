...
//题目2.5
//题目要求：PF0闪烁作为任务1，LED跑马灯及数码管循环显示作为任务2，在SYSTICK的时间调度下，两个任务优先级相同，按时间片运行。
//         现在增加任务3，按下USR_SW1 按键时，PN0常亮，松开按键熄灭。
//         任务 3 的优先级高于任务 1，2，即任务3 在执行时(按下USR_SW1)，任务 1，2 均不执行。
//核心思想：利用SYSTICK，任务1与任务2相同优先级，当SW1按下时，优先执行任务3
...
volatile uint16_t systick_time_ms_couter;
volatile uint8_t systick_time_ms_status = false;
...
int main(void)
{
	...
	while (1)
	{
		if (systick_time_ms_status)
		{
			systick_time_ms_status = 0;
			display_counter += 1;
			if (display_counter > 8)
				display_counter = 1;
			Display1(display_counter); //任务1
			Display2();				   //任务2
		}
	}
}
...
void SysTick_Handler(void)
{
	...
	if (systick_time_ms_couter != 0)
		systick_time_ms_couter--;
	else
	{
		systick_time_ms_couter = 1000;
		systick_time_ms_status = 1;
	}
	if (GPIOPinRead(GPIO_PORTJ_BASE, GPIO_PIN_0) == 0)
	{
		systick_time_ms_status = 0;
		GPIOPinWrite(GPIO_PORTN_BASE, GPIO_PIN_0, GPIO_PIN_0); //任务3，优先级最高
	}
	else
...
void Display1(int counter)
{
	result = I2C0_WriteByte(TCA6424_I2CADDR, TCA6424_OUTPUT_PORT1, seg7[counter]);
	result = I2C0_WriteByte(TCA6424_I2CADDR, TCA6424_OUTPUT_PORT2, led[counter - 1]);
	result = I2C0_WriteByte(PCA9557_I2CADDR, PCA9557_OUTPUT, ~(led[counter - 1]));
}
void Display2()
{
...
void S800_GPIO_Init(void)
{
	...
	SysCtlPeripheralEnable(SYSCTL_PERIPH_GPION); //Enable PortF
